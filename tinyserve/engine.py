...
Usage:
    ...
    from tinyserve.engine import offload_model
...
class OffloadConfig:
    ...
    eviction_policy: str = "lfru"
    temporal_prefetch: bool = True
    max_context_tokens: int = 0
    kv_storage_dtype: torch.dtype = field(default_factory=lambda: torch.float8_e4m3fn)
    vram_fraction: float = 0.90
    compress_weights: bool = True
    mmap_weights: bool = False
    pinned_weight_cache_gb: float = 0
    kv_on_cpu: bool = False
    substitute_table_path: str | None = None
    activation_stats_path: str | None = None
    attention_backend: str | AttentionBackend | None = None
    sliding_window_kv: bool = False
    kv_sink_tokens: int = 4
    kv_window_tokens: int = 1024


TinyserveConfig = OffloadConfig  # backward-compat alias — keep permanently (external API)
...
def _apply_offload_config(offload_config: OffloadConfig, locals_dict: dict) -> dict:
    """Merge OffloadConfig fields into a kwargs dict.

    Config fields set all corresponding local variables; attention_backend
    is only overridden when config provides a non-None value.
    """
    mapping = {
        "cache_capacity": "expert_cache_slots",
        "cache_policy": "eviction_policy",
        "adaptive_fate": "temporal_prefetch",
        "max_seq_len": "max_context_tokens",
        "kv_dtype": "kv_storage_dtype",
        "gpu_memory_utilization": "vram_fraction",
        "fp8": "compress_weights",
        "disk_offload": "mmap_weights",
        "ram_cache_gb": "pinned_weight_cache_gb",
        "kv_offload": "kv_on_cpu",
        "buddy_table_path": "substitute_table_path",
        "imatrix_path": "activation_stats_path",
        "streaming": "sliding_window_kv",
        "streaming_sink_size": "kv_sink_tokens",
        "streaming_window_size": "kv_window_tokens",
    }
    for local_name, config_attr in mapping.items():
        locals_dict[local_name] = getattr(offload_config, config_attr)
    if offload_config.attention_backend is not None:
        locals_dict["attn_implementation"] = offload_config.attention_backend
    return locals_dict
...
def offload_model(
    ...
    Args:
        ...
        offload_config: OffloadConfig object. When provided, its fields override
    ...
    streaming = getattr(offload_config, "sliding_window_kv", False) if offload_config is not None else False
    streaming_sink_size = getattr(offload_config, "kv_sink_tokens", 4) if offload_config is not None else 4
    streaming_window_size = (
        getattr(offload_config, "kv_window_tokens", 1024) if offload_config is not None else 1024
    )
    if max_seq_len > 0:
