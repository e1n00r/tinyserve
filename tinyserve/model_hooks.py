...
_fate = FATEPrefetcher()


def reset_temporal_routing() -> None:
    """Clear temporal routing state. Call before each benchmark run."""
    _fate.reset()
...
def reset_fate_stats() -> None:
    ...
    _fate.stats.clear()
    _fate.pending_predictions.clear()
...
def get_fate_accuracy_by_layer() -> dict[int, dict]:
    ...
    """
    result = {}
    for layer_idx, stats in _fate.stats.items():
...
def _record_fate_prediction(target_layer_idx: int, predicted_set: set) -> None:
    ...
    _fate.pending_predictions[target_layer_idx] = predicted_set
...
def _record_fate_outcome(layer_idx: int, actual_indices: list[int]) -> None:
    ...
    if layer_idx not in _fate.pending_predictions:
        return
    predicted = _fate.pending_predictions.pop(layer_idx)
    actual_set = set(actual_indices)
    hits = len(actual_set & predicted)
    if layer_idx not in _fate.stats:
        _fate.stats[layer_idx] = {"predictions": 0, "hits": 0}
    _fate.stats[layer_idx]["predictions"] += len(actual_set)
    _fate.stats[layer_idx]["hits"] += hits
...
def _install_offloaded_forward(
    ...
    def offloaded_forward(hidden_states, **_kwargs):
        ...
        # when _fate.stats is actively being collected (--fate-diagnostic).
        if _fate.pending_predictions and flat.shape[0] == 1:
        ...
        # Must happen before the prefetch decision below so _fate.access_history is always current.
        ...
        if flat.shape[0] == 1:
            _fate.access_history[layer_idx] = top_idx[0]
        ...
        # the very first token (when _fate.access_history is empty for that layer). Temporal
        ...
        if flat.shape[0] == 1 and next_pipeline is not None and _fate_fn is not None:
            ...
            use_temporal = adaptive_fate and target_layer in _fate.access_history
            if use_temporal:
                predicted = _fate.access_history[target_layer]  # GPU tensor
            else:
        ...
        if flat.shape[0] == 1 and all_pipelines is not None:
            for hop in range(2, 4):
                ...
                if target < len(all_pipelines) and target in _fate.access_history:
                    with _maybe_phase(_prof, "schedule_prefetch"):
                        all_pipelines[target].schedule_prefetch(target, _fate.access_history[target])
